...
int my_sqlite3_callback(void * para, int n_column, char **column_value, char **column_name) 
{ 
	int i=0; 
...
void my_message_callback(struct mosquitto *mosq, void *obj, const struct mosquitto_message *msg)
{
	...
	sprintf(time_now,"%d.%d.%d %d:%d:%d ",lt->tm_year+1900,lt->tm_mon+1,lt->tm_mday,lt->tm_hour,lt->tm_min,lt->tm_sec);
	...
	if (strstr((char *)msg->payload, "Temp is:") != NULL)
	{
		...
		sprintf(buf,"insert into TEMP (time,temp) values('%s',%d);", time_now , temp);
		printf("%s\n\n",buf);
		sqlite3_exec(db,"create table TEMP(id integer primary key,time text,temp integer);",NULL,NULL,&errmsg);
		sqlite3_exec(db,buf,NULL,NULL,&errmsg);
		sqlite3_exec(db,"select * from TEMP;",NULL,NULL,&errmsg);
	...
	else if (strstr((char *)msg->payload, "Humidity is") != NULL)
	{
		...
		sprintf(buf,"insert into HUMI (humi) values(%d);",humi);
		printf("%s\n\n",buf);
		sqlite3_exec(db,"create table HUMI(id integer primary key, humi integer);",NULL,NULL,&errmsg);
		sqlite3_exec(db,buf,NULL,NULL,&errmsg);
		sqlite3_exec(db,"select * from HUMI;",NULL,NULL,&errmsg);
	}
	//发送消息：
